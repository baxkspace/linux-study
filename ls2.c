...
void saveStat(char[], char*, struct stat*);
...
void saveStat(char dirname[], char* filename, struct stat* info_p) {
	...
	if (!root)
		root = tmp;
	else if (strcmp(dirname, "-s") == 0 && tmp->info.st_size < root->info.st_size) {
		tmp->link = root;
		root = tmp;
	}
	else if (strcmp(dirname, "-S") == 0 && tmp->info.st_size > root->info.st_size) {
		...
		if (strcmp(dirname, "-s") == 0) {
			while (curr->link != NULL && tmp->info.st_size > curr->link->info.st_size) 
				curr = curr->link;
		}
...
void dostat(char dirname[], char* filename) {
	...
	if (stat(filename, &info) == -1)
		...
		if (strcmp(dirname, "-s") != 0 && strcmp(dirname, "-S") != 0)
			...
			saveStat(dirname, filename, &info);
	}
}
